...
class SqlPusher {
  ...
  private void GetFiles(File afile) {
    if (afile.isDirectory()) {
      ...
      System.err.println("Skipping non-directory non-file [" + afile +"]");
    }
  }
  ...
  private byte[] ReadFile(File target) {
    try {
            ...
            if (offset != total) {
                System.out.println("Sound the alarm! " + offset +
                    " is the offset but the total is: " + total);
            }
            return totalFile;
        } catch (IOException e) {
  ...
  private boolean PutManifest() throws SQLException {
    ...
    if (maxX > 0 && maxY > 0) {
       System.out.println("Writing manifest file with dims: " + maxX +
                          ", " + maxY);
       PreparedStatement pstmt = null;
       try {
  ...
  public void Process() {
    ...
    final File df = new File(datafileDir);
    GetFiles(df);
    if (files.size() == 0) {
       go = false;
    }
  ...
  public static void Usage() {
    System.out.println(
        "java SqlPusher -f database -d directory [-t tablename]");
